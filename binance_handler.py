...
class BinanceHandler:
    ...
    def open_position(self, side, quantity):
        print(f"Attempting to open {side} position with quantity {quantity}")
    ...
    def close_position(self, side, quantity):
        print(f"Attempting to close {side} position with quantity {quantity}")
        response = self.client.new_order(
            symbol=self.symbol,
            side=side,
            type="MARKET",
        ...
        print(f"Close order response: {response}")
        return response
